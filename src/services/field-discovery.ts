...
export class FieldDiscoveryService {
  ...
  async fetchWorkItemWithAllFields(workItemId: number): Promise<any> {
    ...
    try {
      ...
      console.error(`Failed to fetch work item ${workItemId} with --expand all:`, error);
      throw error;
    }
  }
...
`;
      Array.from(allSystemFields).sort().forEach(field => {
...
`;
      Array.from(allVstsFields).sort().forEach(field => {
        const type = allFieldTypes.get(field) || 'unknown';
        documentation += `- ${field} (${type})\n`;
      });
      documentation += '\n';
...
`;
      Array.from(allMetadataFields).sort().forEach(field => {
        const type = allFieldTypes.get(field) || 'unknown';
        documentation += `- ${field} (${type})\n`;
      });
      documentation += '\n';
...
`;
    ...
    Array.from(typeDistribution.entries()).sort((a, b) => b[1] - a[1]).forEach(([type, count]) => {
  ...
  async discoverAllFields(workItemIds: number[]): Promise<string> {
    ...
    console.log(`Analyzing ${workItemIds.length} work items for field discovery...`);
    ...
    for (const workItemId of workItemIds) {
      try {
        ...
        console.log(`Work item ${workItemId}: Found ${analysis.totalFields} fields`);
      } catch (error) {
    ...
    console.log(`Analysis complete. Successfully analyzed ${analysisResults.length} work items, ${failedFetches} failed.`);
    ...
    if (failedFetches > 0) {
      ...
      return documentation.replace('## Summary' + summarySection, '## Summary' + updatedSummary);
    }
