...
it('should return true when work item has comments and changed after last sync', () => {
  ...
  const result = db.needsCommentSync(workItemId, commentCount, changedDate, lastSyncTime);
...
describe('Azure CLI Error Handling', () => {
  it('should handle authentication timeout', async () => {
    ...
    await expect(client.fetchWorkItems()).rejects.toThrow('Authentication timeout');
...
describe('Database Error Recovery', () => {
  it('should rollback transaction on partial failure', async () => {
    ...
    mockPrisma.workItem.upsert.mockRejectedValueOnce(new Error('Constraint violation'));
  ...
  it('should handle database lock errors', async () => {
    ...
    await expect(db.syncWorkItems([workItem])).rejects.toThrow('Database locked');
