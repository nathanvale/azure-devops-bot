...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log('   pnpm tsx scripts/discover-and-migrate-schema.ts [work-item-ids...]');
  ...
  if (args[0].startsWith('--sample=')) {
    ...
    console.log(`📋 Using ${sampleCount} random work items from your project...\n`);
    ...
    sampleWorkItemIds = args.map(arg => parseInt(arg)).filter(id => !isNaN(id));
  ...
  try {
    ...
    console.log(`   Fetching ${sampleWorkItemIds.length} work items with --expand all...`);
    const documentation = await fieldDiscoveryService.discoverAllFields(sampleWorkItemIds);
    ...
    const result = await migrationService.fullMigrationWorkflow(sampleWorkItemIds);
    ...
    if (result.success) {
      ...
      console.log(`   🔒 Data integrity validated: ${result.dataIntegrityValid}`);
