...
describe("AzureDevOpsBot", () => {
  ...
  describe("Environment Variable Validation", () => {
    it("should exit with error when AZURE_DEVOPS_USER_EMAILS is not set", async () => {
      ...
      await expect(bot.start()).rejects.toThrow('process.exit unexpectedly called with "1"');
      
      expect(processExitSpy).toHaveBeenCalledWith(1);
      expect(consoleSpy.error).toHaveBeenCalledWith('❌ Error: AZURE_DEVOPS_USER_EMAILS environment variable is required');
    });
    ...
    it("should exit with error when AZURE_DEVOPS_USER_EMAILS contains only whitespace", async () => {
      ...
      expect(processExitSpy).toHaveBeenCalledWith(1);
      expect(consoleSpy.error).toHaveBeenCalledWith('❌ Error: At least one valid email address must be provided');
    });
    ...
    it("should parse multiple emails correctly", async () => {
      process.env.AZURE_DEVOPS_USER_EMAILS = 'user1@fwc.gov.au, user2@fwc.gov.au , user3@fwc.gov.au';
      ...
      expect(mockSetUserEmails).toHaveBeenCalledWith(['user1@fwc.gov.au', 'user2@fwc.gov.au', 'user3@fwc.gov.au']);
    });
  ...
  describe("Email Validation", () => {
    beforeEach(() => {
      process.env.AZURE_DEVOPS_USER_EMAILS = 'valid@fwc.gov.au,invalid@example.com';
    });
    ...
    it("should exit with error when some emails are invalid", async () => {
      ...
      expect(processExitSpy).toHaveBeenCalledWith(1);
      expect(consoleSpy.error).toHaveBeenCalledWith('❌ Error: The following email addresses were not found in Azure DevOps:');
      expect(consoleSpy.error).toHaveBeenCalledWith('  • invalid@example.com');
      expect(consoleSpy.error).toHaveBeenCalledWith('Valid emails found:', 'valid@fwc.gov.au');
    });

    it("should exit with error when all emails are invalid", async () => {
      process.env.AZURE_DEVOPS_USER_EMAILS = 'invalid1@example.com,invalid2@example.com';
      ...
      expect(consoleSpy.error).toHaveBeenCalledWith('Valid emails found:', 'None');
    });

    it("should continue when all emails are valid", async () => {
      process.env.AZURE_DEVOPS_USER_EMAILS = 'valid1@fwc.gov.au,valid2@fwc.gov.au';
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith('✅ All email addresses validated successfully: valid1@fwc.gov.au, valid2@fwc.gov.au');
    });
    ...
    it("should show helpful error messages for invalid emails", async () => {
      ...
      expect(consoleSpy.error).toHaveBeenCalledWith('Please ensure these email addresses:');
      expect(consoleSpy.error).toHaveBeenCalledWith('  1. Are correctly spelled');
      expect(consoleSpy.error).toHaveBeenCalledWith('  2. Exist in the Azure DevOps organization');
      expect(consoleSpy.error).toHaveBeenCalledWith('  3. Have access to the project');
    });
  });
  ...
  describe("Application Flow", () => {
    ...
    it("should use 'summary' as default query when no args provided", async () => {
      ...
      expect(mockQueryEngine.processQuery).toHaveBeenCalledWith('summary', ['valid@fwc.gov.au']);
    });
    ...
    it("should use command line arguments as query", async () => {
      ...
      expect(mockQueryEngine.processQuery).toHaveBeenCalledWith('search for bugs', ['valid@fwc.gov.au']);
    });
  ...
  describe("Logging", () => {
    ...
    it("should log validation start message", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith('🔍 Validating email addresses in Azure DevOps: user1@fwc.gov.au, user2@fwc.gov.au');
    });

    it("should log validation success message", async () => {
      await bot.start();
      
      expect(consoleSpy.log).toHaveBeenCalledWith('✅ All email addresses validated successfully: user1@fwc.gov.au, user2@fwc.gov.au');
    });
