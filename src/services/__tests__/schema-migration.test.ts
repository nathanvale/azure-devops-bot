...
describe("SchemaMigrationService", () => {
  ...
  describe("generateComprehensiveSchema", () => {
    it("should generate comprehensive schema with all discovered fields", () => {
      ...
      expect(schema).toContain('customFields      String?  // JSON storage for custom fields');
  ...
  describe("createMigrationFile", () => {
    it("should create migration SQL for new comprehensive fields", () => {
      const newFields = [
        ...
        { name: 'commentCount', type: 'Int?', sqlType: 'INTEGER', defaultValue: '0' },
        { name: 'hasAttachments', type: 'Boolean', sqlType: 'BOOLEAN', defaultValue: 'FALSE' },
      ...
      expect(migrationSql).toContain('ALTER TABLE work_items ADD COLUMN rev INTEGER;');
      expect(migrationSql).toContain('ALTER TABLE work_items ADD COLUMN reason TEXT;');
      expect(migrationSql).toContain('ALTER TABLE work_items ADD COLUMN commentCount INTEGER DEFAULT 0;');
      expect(migrationSql).toContain('ALTER TABLE work_items ADD COLUMN hasAttachments BOOLEAN DEFAULT FALSE;');
      expect(migrationSql).toContain('ALTER TABLE work_items ADD COLUMN stackRank REAL;');
      expect(migrationSql).toContain('ALTER TABLE work_items ADD COLUMN customFields TEXT;');
      
      // Should include performance indexes
      expect(migrationSql).toContain('CREATE INDEX idx_work_items_rev ON work_items(rev);');
      expect(migrationSql).toContain('CREATE INDEX idx_work_items_commentCount ON work_items(commentCount);');
      expect(migrationSql).toContain('CREATE INDEX idx_work_items_hasAttachments ON work_items(hasAttachments);');
    });
  ...
  describe("testMigration", () => {
    it("should test migration with sample data", async () => {
      ...
      mockExecAsync.mockResolvedValueOnce({ stdout: 'Migration applied successfully' });
      // Mock successful rollback
      mockExecAsync.mockResolvedValueOnce({ stdout: 'Migration rolled back successfully' });
  ...
  describe("validateDataIntegrity", () => {
    it("should validate that existing data is preserved after migration", async () => {
      ...
      const postMigrationData = [
        { id: 1234, title: 'Test Item 1', rawJson: '{"id": 1234}', rev: null, reason: null },
        { id: 5678, title: 'Test Item 2', rawJson: '{"id": 5678}', rev: null, reason: null }
      ];

      const result = service.validateDataIntegrity(preMigrationData, postMigrationData);
    ...
    it("should detect missing records after migration", () => {
      ...
      const postMigrationData = [
        { id: 1234, title: 'Test Item 1', rev: null }
      ];
  ...
  describe("fullMigrationWorkflow", () => {
    it("should execute complete migration workflow successfully", async () => {
      ...
      vi.spyOn(service['fieldDiscoveryService'], 'fetchWorkItemWithAllFields')
        .mockResolvedValue(mockWorkItem);
      
      vi.spyOn(service['fieldDiscoveryService'], 'analyzeFields')
        .mockReturnValue({
          totalFields: 3,
    ...
    it("should handle workflow failure at schema generation", async () => {
      ...
      vi.mocked(writeFile).mockRejectedValueOnce(new Error('Schema generation failed'));
    ...
    it("should handle workflow failure at migration creation", async () => {
      ...
      mockExecAsync.mockRejectedValueOnce(new Error('Migration creation failed'));
