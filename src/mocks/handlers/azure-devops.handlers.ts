...
const mockWorkItems = [
  {
    ...
    fields: {
      ...
      'System.Description': 'As a user, I want to authenticate so that I can access the system'
    }
  },
  {
    id: 5678,
...
export const workItemsQueryHandler = http.post(
  ...
  async ({ request }) => {
    ...
    if (query.includes('state = \'active\'')) {
      filteredItems = mockWorkItems.filter(item => 
        item.fields['System.State'] === 'Active'
      )
...
export const createWorkItemHandler = http.post(
  ...
  async ({ request, params }) => {
    ...
    const descriptionOp = body.find(op => op.path === '/fields/System.Description')
