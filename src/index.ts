...
export class AzureDevOpsBot {
  ...
  private getUserEmails(): string[] {
    ...
    if (!envEmails) {
      console.error('❌ Error: AZURE_DEVOPS_USER_EMAILS environment variable is required');
      console.error('');
      console.error('Please set the email address(es) that your Azure DevOps account uses for work item assignment:');
      console.error('Example: AZURE_DEVOPS_USER_EMAILS=Jane.Citizen@fwc.gov.au,ITEX-JC@fwc.gov.au');
      console.error('');
      console.error('You can set this environment variable in several ways:');
      console.error('');
      console.error('1. Create a .env file in your project root:');
      console.error('   echo "AZURE_DEVOPS_USER_EMAILS=Jane.Citizen@fwc.gov.au,ITEX-JC@fwc.gov.au" >> .env');
      console.error('');
      console.error('2. Export directly in your shell:');
      console.error('   export AZURE_DEVOPS_USER_EMAILS=Jane.Citizen@fwc.gov.au,ITEX-JC@fwc.gov.au');
      console.error('');
      console.error('3. Set permanently in your shell profile (~/.bashrc, ~/.zshrc, etc.):');
      console.error('   echo "export AZURE_DEVOPS_USER_EMAILS=Jane.Citizen@fwc.gov.au,ITEX-JC@fwc.gov.au" >> ~/.zshrc');
      console.error('');
      console.error('4. Run with the environment variable inline:');
      console.error('   AZURE_DEVOPS_USER_EMAILS=Jane.Citizen@fwc.gov.au,ITEX-JC@fwc.gov.au pnpm dev');
      process.exit(1);
    }
    
    const emails = envEmails.split(',').map(email => email.trim()).filter(email => email.length > 0);
  ...
  async start(): Promise<void> {
    try {
      ...
      console.log(`🔍 Validating email addresses in Azure DevOps: ${this.userEmails.join(', ')}`);
      
      // Validate emails exist in Azure DevOps
      const azureClient = new AzureDevOpsClient();
      const { valid, invalid } = await azureClient.validateUserEmails(this.userEmails);
      
      if (invalid.length > 0) {
        console.error('❌ Error: The following email addresses were not found in Azure DevOps:');
        ...
        console.error('');
        console.error('Valid emails found:', valid.length > 0 ? valid.join(', ') : 'None');
        process.exit(1);
      }
