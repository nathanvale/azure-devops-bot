...
- ✅ Full data sync using `az boards work-item show --expand all` 
...
- **Azure Integration**: Azure CLI (`az boards`) with structured JSON parsing
...
- **Azure CLI Integration**: Enhancing `az boards` command integration or error handling
...
- **Multi-Domain Features**: Work spanning MCP server, database, sync service, and Azure CLI layers
...
4. **Architecture Impact**: Spans MCP server, database service, Azure CLI integration, and testing layers
...
- ✅ **Full Data Sync**: `az boards work-item show --expand all` integration
...
- **Service Layer Architecture**: MCP Server → Database Service → Sync Service → Azure CLI
- **Azure CLI Integration**: `az boards` commands with structured JSON parsing
- **Transport Protocol**: stdio for MCP 1.15+ communication
- **Data Flow**: Azure DevOps → CLI JSON → Prisma ORM → SQLite → MCP JSON Response
...
5. **Architecture Consistency**: Maintain MCP → Database → Sync → Azure CLI layer separation
...
- **Azure DevOps Integration Expert**: `az boards` CLI commands, JSON parsing, error handling
...
- **Azure CLI Integration**: Proper error handling and structured JSON parsing
